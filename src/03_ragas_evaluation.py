...
# RAGAS 0.4.x still imports this legacy LangChain community module.  Recent
# langchain-community releases removed it, although RAGAS only needs the class
# for an internal OpenAI capability check.  Register a compatible alias before
# importing RAGAS so OpenAI-based evaluation remains available.
try:
    from langchain_community.chat_models.vertexai import ChatVertexAI  # noqa: F401
except ModuleNotFoundError:
    from langchain_community.llms import VertexAI

    _vertexai_compat = types.ModuleType("langchain_community.chat_models.vertexai")
    _vertexai_compat.ChatVertexAI = VertexAI
    sys.modules["langchain_community.chat_models.vertexai"] = _vertexai_compat
...
SYSTEM_V1 = (
    ...
    "Keep the response concise, factual, and limited to 2-4 sentences. "
...
SYSTEM_V2 = (
    "You are an AI subject-matter expert. Use only the supplied context, first "
    "identify the relevant facts, then give a clear and well-organized answer in "
    "3-5 sentences. Explain important terms when useful and do not invent facts "
    "outside the context. If the context is insufficient, explicitly state that.\n\n"
    "Context:\n{context}"
)
